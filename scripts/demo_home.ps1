...
$containerName = 'pruefversand-demo-home-{0}' -f $suffix
...
try {
    ...
    python -m pruefversand `
        ...
        demo-home
    if ($LASTEXITCODE -ne 0) {
        throw 'La commande demo-home a echoue.'
    }
    $emlCountAfterFirst = @(
        Get-ChildItem -LiteralPath 'var/mock_mail' -Filter '*.eml' `
            -File -ErrorAction SilentlyContinue
    ...
    if ($ReplayTwice) {
        python -m pruefversand `
            ...
            demo-home
        if ($LASTEXITCODE -ne 0) {
            throw 'La relance demo-home a echoue.'
        }
        $emlCountAfterSecond = @(
            Get-ChildItem -LiteralPath 'var/mock_mail' -Filter '*.eml' `
                -File -ErrorAction SilentlyContinue
        ).Count
        if ($emlCountAfterSecond -ne $emlCountAfterFirst) {
            throw 'La relance a cree un fichier .eml en doublon.'
        }
        Write-Output (
